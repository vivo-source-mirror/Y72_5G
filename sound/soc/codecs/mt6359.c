...
/* ADD by wangkai, for bottom noise when fast charging */
...
void changeFSM()
{
	printk("%s, mic record status %d, VOIP status %d\n", __func__, mic_switch_flag, voip_switch_flag);
	if (mic_switch_flag == true && voip_switch_flag == false)
			  ...
			  int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		if (mic_type == priv->mux_select[MUX_MIC_TYPE_1]) {
